...
import \{ View, Text, SafeAreaView, ScrollView, TouchableOpacity, FlatList, Alert, RefreshControl, TextInput \} from "react-native";
...
const supportedProviders = \{
  reasoning: [
    \{ provider: 'moonshot', model: 'kimi-k2-0905' \},
  ...
  asr: [
    ...
    \{ provider: 'huggingface', model: 'whisper-large-v3' \},
  ],
  tts: [
    \{ provider: 'huggingface', model: 'suno/bark' \},
  ],
  visuals: [
    \{ provider: 'huggingface', model: 'stable-diffusion-v1.0' \},
  ],
  animations: [
    \{ provider: 'huggingface', model: 'animatediff' \},
    \{ provider: 'remotion', model: 'manim' \},
  ],
  avatars: [
    \{ provider: 'huggingface', model: 'sadtalker' \},
  ],
\};
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const \{ data: settingsData, error: settingsError \} = await supabase
        .from\('model_settings'\)
        .select\('category, provider, model_name'\)
        .eq\('is_active', true\);
      ...
      const settingsWithKeys = settingsData?.map\(setting => \{
        ...setting,
        api_key: '' // Don't fetch actual keys for security
      \}\) \|\| \[\];

      setModelSettings\(settingsWithKeys\);
      setApiKeys\(keysData?.map\(k => \(\{ provider: k.provider, key: '' \}\)\) \|\| \[\]\);
  ...
  const saveApiKeys = async \(\) => \{
    try \{
      for \(const key of apiKeys\) \{
        if \(key.key\) \{ // Only save if key is provided
          const \{ error \} = await supabase
            .from\('api_keys'\)
            .upsert\(\{
              provider: key.provider,
              encrypted_key: supabase.rpc\('encrypt_api_key', \{ plain_key: key.key \}\)
            \}\);
          if \(error\) throw error;
        \}
      \}
